...
} from "./content/rendered_fetch.ts";
import { runRenderedRecovery } from "./content/rendered_recovery.ts";
...
const DEFAULT_ENQUEUE_DUE_LIMIT = 25;
const RETRY_DELAYS_SECONDS = [900, 3600];
...
const batchSize = envNumber(
  "CONTENT_RENDER_RECOVERY_BATCH_SIZE",
  DEFAULT_BATCH_SIZE,
);
const visibilityTimeoutSeconds = envNumber(
  "CONTENT_RENDER_RECOVERY_VISIBILITY_TIMEOUT_SECONDS",
  DEFAULT_VISIBILITY_TIMEOUT_SECONDS,
);
const staleAfterSeconds = envNumber(
  "CONTENT_RENDER_RECOVERY_STALE_AFTER_SECONDS",
  DEFAULT_STALE_AFTER_SECONDS,
);
const maxRetries = envNumber(
  "CONTENT_RENDER_RECOVERY_RETRY_LIMIT",
  DEFAULT_RETRY_LIMIT,
);
const enqueueDueLimit = envNumber(
  "CONTENT_RENDER_RECOVERY_ENQUEUE_LIMIT",
...
function getSupabase(): ReturnType<typeof createClient<any>> {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error(
      "SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set for rendered content recovery",
    );
  }
...
export async function processContentRenderRecoveryBatch(): Promise<
  BatchResult
> {
  ...
  if (!isRenderedFetchConfigured()) {
    console.warn(
      "rendered content recovery is disabled because CONTENT_RENDERER_URL or CONTENT_RENDERER_SECRET is missing",
    );
    return result;
  }
  ...
  const { data: enqueuedDue, error: enqueueDueError } = await supabase.rpc(
    "enqueue_due_content_render_recoveries",
    { p_limit: enqueueDueLimit },
  );
  if (enqueueDueError) {
    throw new Error(
      `Failed to enqueue due rendered content recoveries: ${enqueueDueError.message}`,
    );
  }
  result.enqueued_due = Number.isFinite(enqueuedDue) ? Number(enqueuedDue) : 0;

  const { data, error } = await supabase.rpc(
    "dequeue_content_render_recovery",
    {
  ...
  if (error) {
    throw new Error(
      `Failed to read rendered content recovery queue: ${error.message}`,
    );
  }
  ...
  for (const queueMessage of queueMessages) {
    try {
      ...
      result.failed += 1;
      console.error("rendered content recovery message failed unexpectedly", {
...
export async function enqueueContentRenderRecovery(
  contentId: string,
  trigger: "save" | "retry" | "cron" | "escalate" = "save",
  delaySeconds = 0,
  retryCount = 0,
): Promise<void> {
  const supabase = getSupabase();
  const { error } = await supabase.rpc("enqueue_content_render_recovery", {
  ...
  if (error) {
    throw new Error(
      `Failed to enqueue rendered content recovery for ${contentId}: ${error.message}`,
    );
  }
}

export async function invokeContentRenderRecoveryProcessor(payload: {
  content_id?: string;
  trigger?: string;
} = {}): Promise<void> {
  const supabase = getSupabase();
  const { data, error } = await supabase.rpc(
    "invoke_content_render_recovery_processor",
    {
      p_payload: payload,
    },
  );
  if (error) {
    throw new Error(
      `Failed to invoke rendered content recovery processor: ${error.message}`,
    );
  }

  if (data === null) {
    console.warn(
      "rendered content recovery processor invoke skipped because required Vault secrets are missing",
      payload,
    );
  }
}
...
async function processQueueMessage(
  ...
  try {
    const rendered = await fetchRenderedDocument(
      claimed.resolved_url ?? claimed.canonical_url,
    );
    const recovery = await runRenderedRecovery({
      fetched: rendered,
      current: {
    ...
    if (recovery.kind === "persist") {
      await persistRecoveredContent(claimed, rendered, recovery.processed, {
        parserRecoveryStatus: recovery.recoveryStatus,
        parserRecovery: recovery.recoveryDecision,
      });
      if (recovery.recoveryStatus === "succeeded") {
        result.recovered += 1;
      } else {
        result.dismissed += 1;
      }
    } else {
      await persistRecoveryDismissed(claimed.id, recovery.recoveryDecision);
      result.dismissed += 1;
    }
    ...
    if (failure.retryable && retryCount < maxRetries) {
      ...
      await enqueueContentRenderRecovery(
        claimed.id,
        "retry",
        retryDelaySeconds,
        retryCount + 1,
...
async function claimRecovery(
  ...
  const supabase = getSupabase();
  const { data, error } = await supabase.rpc("claim_content_render_recovery", {
    p_content_id: contentId,
    p_stale_after_seconds: staleAfterSeconds,
  });
  if (error) {
    throw new Error(
      `Failed to claim rendered content recovery ${contentId}: ${error.message}`,
    );
  }
...
async function persistRecoveredContent(
  ...
  input: {
    parserRecoveryStatus: "succeeded" | "dismissed";
  ...
  if (error) {
    throw new Error(
      `Failed to persist rendered recovered content ${claimed.id}: ${error.message}`,
    );
  }
}
...
async function persistRecoveryDismissed(
  ...
  if (error) {
    throw new Error(
      `Failed to persist rendered recovery dismissal ${contentId}: ${error.message}`,
    );
  }
}
...
async function persistRecoveryFailure(
  ...
  if (error) {
    throw new Error(
      `Failed to persist rendered content recovery failure ${contentId}: ${error.message}`,
    );
  }
}
...
async function archiveQueueMessage(msgId: number): Promise<void> {
  const supabase = getSupabase();
  const { error } = await supabase.rpc("archive_content_render_recovery", {
    p_msg_id: msgId,
  });
  if (error) {
    throw new Error(
      `Failed to archive rendered content recovery queue message ${msgId}: ${error.message}`,
    );
  }
}
