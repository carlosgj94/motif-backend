...
const PARSER_NAME = "bloomberg-article";

export async function processBloombergArticle(
  ...
  const ranked = selectPreferredArticleQualityCandidate({
    preferred: selected
      ? {
        id: `bloomberg:${selected.strategyId}`,
  ...
  if (blocks.length === 0) {
    throw ProcessingFailure.parse(
      "Could not recover readable Bloomberg article content",
      {
  ...
  const parserDiagnostics = buildParserDiagnostics({
    ...
    candidates: qualityCandidates.map((candidate) => ({
      ...
      sourceKind: "article",
      notes: candidate.id.startsWith("bloomberg:")
        ? ["provider-specific"]
        : ["generic-fallback"],
    })),
  });
...
function trimBloombergTitle(value: string | null | undefined): string | null {
  if (!value) {
    return null;
  }

  return value.replace(/\s*-\s*Bloomberg\s*$/i, "").trim() || null;
}
